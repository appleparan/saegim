...
export const CATEGORY_ATTRIBUTES: Partial<
  Record<BlockCategoryType, readonly AttributeFieldDef[]>
> = {
  text_block: [
    { key: 'text_language', label: '텍스트 언어', type: 'select', options: TEXT_LANGUAGES },
    { key: 'text_background', label: '배경', type: 'select', options: TEXT_BACKGROUNDS },
    { key: 'text_rotate', label: '회전', type: 'select', options: TEXT_ROTATIONS },
  ],
  title: [
    { key: 'text_language', label: '텍스트 언어', type: 'select', options: TEXT_LANGUAGES },
    { key: 'text_background', label: '배경', type: 'select', options: TEXT_BACKGROUNDS },
  ...
  table: [
    { key: 'table_layout', label: '테이블 방향', type: 'select', options: TABLE_LAYOUTS },
    { key: 'with_span', label: '병합 셀', type: 'toggle' },
    { key: 'line', label: '선 스타일', type: 'select', options: TABLE_LINES },
    { key: 'language', label: '테이블 언어', type: 'select', options: TABLE_LANGUAGES },
  ...
  equation_isolated: [
    { key: 'formula_type', label: '수식 유형', type: 'select', options: FORMULA_TYPES },
    { key: 'equation_language', label: '수식 언어', type: 'select', options: EQUATION_LANGUAGES },
  ],
}
