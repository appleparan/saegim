...
describe('findHoveredImageBlock', () => {
  ...
  it('returns anno_id when cursor is over a figure', () => {
    ...
    const result = findHoveredImageBlock({ x: 350, y: 500 }, containerRect, 0, 0, 1, elements)
    expect(result).toBe(1)
  })

  it('returns anno_id when cursor is over a table', () => {
    // imagePoint ~= (550, 175) → screenPoint = (650, 225)
    const result = findHoveredImageBlock({ x: 650, y: 225 }, containerRect, 0, 0, 1, elements)
    expect(result).toBe(2)
  })

  it('returns null when cursor is over a text block', () => {
    // imagePoint ~= (100, 20) → screenPoint = (200, 70)
    const result = findHoveredImageBlock({ x: 200, y: 70 }, containerRect, 0, 0, 1, elements)
    expect(result).toBeNull()
  })

  it('returns null when cursor is over empty space', () => {
    const result = findHoveredImageBlock({ x: 150, y: 200 }, containerRect, 0, 0, 1, elements)
    expect(result).toBeNull()
  })
  ...
  it('accounts for zoom and pan offset', () => {
    ...
    const result = findHoveredImageBlock({ x: 620, y: 980 }, containerRect, 20, 30, 2, elements)
    expect(result).toBe(1)
  })

  it('returns null when elements list is empty', () => {
    const result = findHoveredImageBlock({ x: 200, y: 200 }, containerRect, 0, 0, 1, [])
    expect(result).toBeNull()
  })
})
