...
import { rectsOverlap, findOverlappingBbox, selectionToImageRect } from '$lib/utils/text-selection'
...
describe('findOverlappingBbox', () => {
  ...
  it('returns null when no elements overlap', () => {
    ...
    const elements = [makeElement(0, 10, 10, 100, 30), makeElement(1, 10, 50, 100, 30)]
  ...
  it('returns the anno_id of the overlapping element', () => {
    ...
    const elements = [makeElement(0, 10, 10, 100, 30), makeElement(1, 10, 200, 100, 30)]
...
describe('selectionToImageRect', () => {
  ...
  it('returns image-space rect when selection is valid', () => {
    ...
    const mockRange = {
      ...
      getClientRects: () => [{ left: 120, top: 60, right: 200, bottom: 80 } as DOMRect],
    }
  ...
  it('accounts for scale when converting to image coordinates', () => {
    ...
    const mockRange = {
      commonAncestorContainer: textNode,
      getClientRects: () => [{ left: 40, top: 20, right: 140, bottom: 60 } as DOMRect],
    }
