...
Tests connectivity to Gemini API or vLLM server without sending images.
"""
...
def check_ocr_connection(ocr_config: dict[str, Any]) -> tuple[bool, str]:
    """Test OCR provider connection based on configuration.

    Args:
        ocr_config: OCR config dict with 'provider' key.
    ...
    if provider == 'gemini':
        ...
        return check_gemini_connection(gemini_config)
