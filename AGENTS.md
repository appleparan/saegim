...
| **OCR 엔진** | 다중 인스턴스 레지스트리 + Strategy 패턴 (commercial_api, vllm, split_pipeline + pdfminer 폴백) |
| **배포** | Docker Compose |
...
- 다중 인스턴스 엔진 레지스트리 (`BaseOCREngine` ABC + Strategy 패턴) (PR #12, PR #65)
...
- ✅ JWT 인증 (access token in-memory + refresh token HttpOnly cookie + token rotation) (PR #88)
- ✅ 시스템 역할 (admin/annotator/reviewer), 최초 유저 → admin 자동 부여 (PR #88)
- ✅ 프론트엔드 로그인/회원가입, auth guard, 자동 토큰 갱신 (PR #95)
- ✅ 관리자 대시보드 — 유저/프로젝트/시스템 통계 관리 (PR #102)
- ✅ 프로젝트 멤버 관리 — CRUD, 멤버십 기반 접근 제어, 프론트엔드 UI (PR #92)
- ✅ 태스크 워크플로우 API — 할당/제출/승인·반려, locked_at 30분 타임아웃 (PR #87)
- ✅ 프론트엔드 태스크 대시보드, 검수 큐, 상태 배지 (PR #91)
- ✅ 작업 현황 보드 — 프로젝트 전체 진행률, 문서별/멤버별 현황 (PR #107)
